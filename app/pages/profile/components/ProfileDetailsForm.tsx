"use client";
import { LuCar, LuCreditCard, LuLock, LuMail, LuMapPin, LuPhone } from "react-icons/lu";
...
export default function ProfileDetailsForm({ detailsForm, onChange, onSubmit, onBack, saveMessage }: ProfileDetailsFormProps) {
  ...
  return (
    <section className="profileDetails" aria-label="Mine oplysninger formular">
      ...
      >
        ...
        <label className="detailsField" htmlFor="details-payment">
          ...
          <div className="detailsFieldValueRow">
            <button
              ...
              id="details-payment"
              className="detailsFieldInput"
              style={{ display: "flex", alignItems: "center", gap: "0.5rem", cursor: isEditing ? "pointer" : "default" }} //added cursor: isEditing ? "pointer" : "default"
              onClick={() => isEditing && setShowPopUp(true)} //added
            >
              {(() => {
        ...
        {!isEditing ? (
          <button type="button" className="detailsSaveButton" onClick={() => setIsEditing(true)}>
            Rediger
          </button>
        ) : (
          <div style={{ display: "flex", gap: "10px" }}>
            ...
            <button type="button" className="detailsSaveButton detailsConfirm" onClick={() => setIsEditing(false)}>
              Annuller
            </button>
          </div>
        )}
        ...
        <div className="betalingsOverlay">
          <div className="batalingPopup">
            ...
            {methods.map((method) => (
              ...
              >
                <img src={`http://localhost:80/icons/${method.payment_gateway_icon_path}`} alt={method.payment_gateway_name} style={{ width: "40px", height: "40px", objectFit: "contain" }} />
                <input type="radio" name="paymentMethod" checked={detailsForm.paymentMethod === method.payment_gateway_id} readOnly />
                <h3>{method.payment_gateway_name}</h3>
              </div>
            ))}
            <button type="button" className="detailsSaveButton" onClick={() => setShowPopUp(false)}>
  Gem
</button>
            {/* <button type="submit" className="detailsSaveButton detailsCancel">
              Gem
